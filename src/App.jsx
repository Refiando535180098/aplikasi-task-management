...
// ==========================================
// 1. DATABASE SEMENTARA (MOCK DATA)
// ==========================================
const INITIAL_DIVISIONS = ['Pusat', 'Direksi', 'Operasional', 'Marketing', 'IT', 'Sales'];

const INITIAL_USERS = [
  { id: 100, nik: 'ADM001', password: 'password123', name: 'System Admin', role: 'admin', avatar: 'SA', division: 'Pusat', position: 'Super Admin' },
  { id: 99, nik: 'DIR001', password: 'password123', name: 'Bapak Direktur', role: 'direksi', avatar: 'BD', division: 'Direksi', position: 'Direktur Utama' },
  { id: 1, nik: 'MGR001', password: 'password123', name: 'Budi Santoso', role: 'manager', avatar: 'BS', division: 'Operasional', position: 'Manager Operasional' },
  { id: 2, nik: 'MGR002', password: 'password123', name: 'Anita Wijaya', role: 'manager', avatar: 'AW', division: 'Marketing', position: 'Manager Marketing' },
  { id: 3, nik: 'MGR003', password: 'password123', name: 'Handoko', role: 'manager', avatar: 'HD', division: 'IT', position: 'IT Manager' },
  { id: 4, nik: 'MGR004', password: 'password123', name: 'Diana Putri', role: 'manager', avatar: 'DP', division: 'Sales', position: 'Sales Manager' },
  { id: 5, nik: 'STF001', password: 'password123', name: 'Siti Aminah', role: 'staff', avatar: 'SA', division: 'Marketing', position: 'Social Media Specialist' },
  { id: 6, nik: 'STF002', password: 'password123', name: 'Rudi Hermawan', role: 'staff', avatar: 'RH', division: 'IT', position: 'Network Admin' },
  { id: 7, nik: 'STF003', password: 'password123', name: 'Dewi Lestari', role: 'staff', avatar: 'DL', division: 'Sales', position: 'B2B Sales' },
  { id: 8, nik: 'STF004', password: 'password123', name: 'Ahmad Faisal', role: 'staff', avatar: 'AF', division: 'Operasional', position: 'Staff Logistik' },
];

const INITIAL_TASKS = [
  { id: 10, title: 'Draft Konten Sosmed Feb W1', description: 'Buat 5 draft konten Instagram untuk minggu pertama.', assignedTo: [5], assignedBy: 2, status: 'done', priority: 'high', dueDate: '2026-02-05', comments: [], attachments: [] },
  { id: 11, title: 'Setup Iklan Meta Ads (Valentine)', description: 'Siapkan materi dan targeting.', assignedTo: [5], assignedBy: 2, status: 'done', priority: 'high', dueDate: '2026-02-08', comments: [], attachments: [] },
  { id: 12, title: 'Laporan Performa Sosmed Jan 2026', description: 'Rekap insight Instagram.', assignedTo: [5], assignedBy: 2, status: 'done', priority: 'medium', dueDate: '2026-02-10', comments: [], attachments: [] },
  { id: 13, title: 'Draft Konten Sosmed Feb W2', description: 'Buat konten.', assignedTo: [5], assignedBy: 2, status: 'done', priority: 'medium', dueDate: '2026-02-12', comments: [], attachments: [] },
  { id: 14, title: 'Live Streaming Promo', description: 'Host sesi live.', assignedTo: [5], assignedBy: 2, status: 'done', priority: 'high', dueDate: '2026-02-14', comments: [], attachments: [] },
  { id: 17, title: 'Riset Kompetitor Q1', description: 'Analisa pergerakan campaign.', assignedTo: [5], assignedBy: 2, status: 'in-progress', priority: 'low', dueDate: '2026-03-05', comments: [], attachments: [] },
  { id: 18, title: 'Persiapan Campaign Ramadhan', description: 'Brainstorming ide awal.', assignedTo: [5], assignedBy: 2, status: 'pending', priority: 'high', dueDate: '2026-03-10', comments: [], attachments: [] },
  { id: 1, title: 'Maintenance Server Pusat', description: 'Update patch OS.', assignedTo: [6], assignedBy: 3, status: 'in-progress', priority: 'high', dueDate: '2026-03-05', comments: [], attachments: [] },
  { id: 2, title: 'Rekap Penjualan Q1', description: 'Pastikan angka.', assignedTo: [7], assignedBy: 4, status: 'pending', priority: 'high', dueDate: '2026-03-15', comments: [], attachments: [] },
];

const INITIAL_NOTIFICATIONS = [
  { id: 1, userId: 5, type: 'task', message: 'Tugas baru: Persiapan Campaign Ramadhan', read: false, time: '09:00', taskId: 18 },
];
...
export default function App() {
  ...
  const [notifications, setNotifications] = useState([]); // Mulai dengan notifikasi kosong 
  ...
  const [activeChatId, setActiveChatId] = useState(null); // Penyimpan ID chat yang sedang dibuka
  ...
  const [editDivInput, setEditDivInput] = useState(''); 
  ...
  // FIX: Nilai default Laporan adalah "ALL"
  const [reportTargetUserId, setReportTargetUserId] = useState('ALL'); 

  // --- TAMBAHAN KODE: State untuk fitur filter bulan dan tanggal ---
  ...
  // --- FUNGSI UPLOAD LAMPIRAN YANG HILANG ---
  // --- FUNGSI UPLOAD FILE KE SERVER & DATABASE ---
  const handleFileUpload = async (e) => {
    ...
    // Menggunakan FormData untuk mengirim file fisik
    ...
    try {
      const response = await fetch('http://localhost/api_task/upload_attachment.php', {
        method: 'POST',
        body: formData
      });
      ...
      if (result.status === 'success') {
        const updatedTasks = tasks.map(t => {
          if (t.id === selectedTask.id) {
            const currentAttachments = Array.isArray(t.attachments) ? t.attachments : [];
            const updatedTask = { ...t, attachments: [...currentAttachments, result.data] };
      ...
      alert('Error terhubung ke server saat upload.');
      console.error(err);
    }
  ...
  const handleDeleteAttachment = async (attachmentId, fileName) => {
    ...
    try {
      const response = await fetch('http://localhost/api_task/delete_attachment.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ taskId: selectedTask.id, attachmentId: attachmentId })
      });
      ...
      if(result.status === 'success') {
        const updatedTasks = tasks.map(t => {
          if (t.id === selectedTask.id) {
            const filtered = (t.attachments || []).filter(a => a.id !== attachmentId);
            const updatedTask = { ...t, attachments: filtered };
            setSelectedTask(updatedTask);
            return updatedTask;
          }
          return t;
  ...
  // --- FUNGSI KIRIM CHAT (MENDUKUNG PUSAT PESAN & MODAL) ---
  const handleAddComment = async (e) => {
    e.preventDefault();
    if (!newComment.trim()) return;

    // Deteksi cerdas: Target pengiriman (Dari tab Chat atau Modal)
    ...
    const updatedTasks = tasks.map(t => {
      if (t.id === targetTaskId) {
        ...
        const updatedTask = { ...t, comments: updatedCommentsArray };
        
        // Kirim Notifikasi
        const relevantUserIds = new Set([...getAssigneesArray(t.assignedTo), t.assignedBy]);
        relevantUserIds.forEach(targetUserId => {
          if (targetUserId && targetUserId !== currentUser.id) {
             addNotification(targetUserId, 'chat', `Pesan baru dari ${currentUser.name} di tugas: "${t.title}"`, t.id);
          }
        });
    ...
    // Proses ke MySQL
    try {
      await fetch('http://localhost/api_task/update_comments.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ taskId: targetTaskId, comments: updatedCommentsArray })
      });
    } catch (err) {
  ...
  const handleSaveConfig = async () => {
    try {
      const response = await fetch('http://localhost/api_task/update_settings.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const result = await response.json();
      if (result.status === 'success') {
        setSysConfig(configForm); // Update tampilan langsung
  ...
  const handleStatusUpdate = async (taskId, newStatus) => {
    ...
    try {
      const response = await fetch('http://localhost/api_task/update_status.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      try { 
        ...
        console.error("Bukan JSON, ini balasan PHP:", textResult);
        return alert("PHP Error saat Update Status! Cek Console (F12).");
      }
      
      if (result.status !== 'success') {
        alert("Gagal mengupdate database: " + result.message);
      }
    } catch (error) {
      alert("Cek folder XAMPP: Pastikan file update_status.php sudah ada!");
      console.error(error);
    }
  };
  ...
  const loadTasksFromDB = async () => {
    try {
      const response = await fetch('http://localhost/api_task/get_tasks.php');
      const result = await response.json();
      if (result.status === 'success' && Array.isArray(result.data)) {
        setTasks(result.data);
        if (currentUser) {
          const myUnfinishedTasks = result.data.filter(t => 
          ...
          if (myUnfinishedTasks.length > 0) {
            setNotifications(prev => {
              ...
              myUnfinishedTasks.forEach(task => {
                // Cek agar tidak membuat notifikasi berulang kali (spam)
                ...
                if (!isAlreadyNotified) {
                  currentNotifs.unshift({
                    ...
                    userId: currentUser.id,
                    type: 'reminder',
                    message: `PENGINGAT: Tugas "${task.title}" belum dikerjakan!`, // Pesan Pengingat
                    read: false,
  ...
  // Jalankan fungsi di atas otomatis SAAT BARU LOGIN
  useEffect(() => {
    if (currentUser) {
      loadTasksFromDB();
    }
  ...
  useEffect(() => {
    // Ambil NIK dan Password dari penyimpanan browser
    const activeSession = localStorage.getItem('syntegra_user_session');
    if (activeSession) {
      setCurrentUser(JSON.parse(activeSession)); // Langsung Login-kan user!
    }

    // Ambil NIK dan Password dari penyimpanan browser (untuk form)
  ...
  // --- FITUR BARU: HAPUS NOTIF OTOMATIS SAAT TUGAS/CHAT DIBACA ---
  useEffect(() => {
    if (selectedTask) {
      // Menyaring dan membuang semua notifikasi yang memiliki taskId sama dengan tugas yang sedang dibuka
      setNotifications(prev => prev.filter(n => n.taskId !== selectedTask.id));
    }
  }, [selectedTask]);

  // --- FITUR BARU: TARIK PENGATURAN DARI DATABASE ---
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        const response = await fetch('http://localhost/api_task/get_settings.php');
        const result = await response.json();
        if (result.status === 'success') {
          setSysConfig(result.data);
          setConfigForm(result.data); // Samakan form dengan data asli
        }
      } catch (error) {
    ...
    fetchSettings();
  }, []); // Jalan 1x saat website pertama kali dimuat

  useEffect(() => {
    if (chatEndRef.current) chatEndRef.current.scrollIntoView({ behavior: 'smooth' });
  }, [selectedTask?.comments]);

  // --- LOGIKA AMAN ARRAY & STRING ---
  const getAssigneesArray = (assignedTo) => {
    if (!assignedTo) return [];
    // Jika MySQL mengirimkan format String JSON misal "[5]"
    if (typeof assignedTo === 'string') {
      try {
        // Ubah string JSON jadi array, lalu pastikan isinya diubah jadi Angka (Number)
        return JSON.parse(assignedTo).map(Number);
      } catch (e) {
        return [Number(assignedTo)];
      }
    }
    // Jika sudah berupa Array, pastikan isinya Angka
  ...
  //   e.preventDefault();
  //   const user = users.find(u => u.nik === loginNik && u.password === loginPassword);
  //   if (user) {
  ...
  //       localStorage.setItem('syntegra_saved_password', loginPassword); // Simpan password
  //     } else {
  //       localStorage.removeItem('syntegra_saved_nik');
  //       localStorage.removeItem('syntegra_saved_password'); // Hapus password
  //     }
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      // Mengirim data ke PHP API di XAMPP
      const response = await fetch('http://localhost/api_task/login.php', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ nik: loginNik, password: loginPassword })
      });
      ...
      if (data.status === 'success') {
        ...
        setLoginError(data.message || 'NIK atau Password yang Anda masukkan salah!');
      }
    } catch (error) {
  ...
  const addNotification = (userId, type, message, taskId) => {
    const newNotif = { id: Date.now(), userId, type, message, read: false, time: new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }), taskId };
    setNotifications(prev => [newNotif, ...prev]);
  };

  // --- TASK LOGIC ---
  // --- TAMBAHAN: Tarik Data Tasks dari MySQL ---
  useEffect(() => {
    if (currentUser) {
  ...
  }, [currentUser]); // Berjalan otomatis saat login
  // --- TASK LOGIC (VERSI DATABASE) ---
  const handleCreateTask = async (e) => {
    ...
    try {
      // Mengirim POST Request ke insert_task.php
      const response = await fetch('http://localhost/api_task/insert_task.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      catch (e) { return alert("Error PHP: " + textResult); }

      if (result.status === 'success') {
        // Hapus pop-up alert yang mengganggu jika mau, atau biarkan
        setIsModalOpen(false);
        setNewTask({ title: '', description: '', assignedTo: [], priority: 'medium', dueDate: '' });
        
        // PANGGIL FUNGSI INI AGAR DATA MUNCUL TANPA RELOAD/LOGOUT
        loadTasksFromDB(); 
      } else {
        alert("Gagal menyimpan: " + result.message);
      }
    } catch (error) {
      alert("Error gagal terhubung ke server saat menyimpan tugas.");
      console.error(error);
    }
  };
  ...
  const handleAddDivision = () => {
    if(newDivName.trim() && !divisions.includes(newDivName.trim())) { setDivisions([...divisions, newDivName.trim()]); setNewDivName(''); }
  };
  const handleUpdateDivision = (oldName) => {
    if(!editDivInput.trim() || editDivInput.trim() === oldName) return setEditingDiv(null);
    const newName = editDivInput.trim();
    setDivisions(divisions.map(d => d === oldName ? newName : d));
    setUsers(users.map(u => u.division === oldName ? { ...u, division: newName } : u));
    setEditingDiv(null);
  };
  const handleDeleteDivision = (divName) => {
    if(window.confirm(`Hapus divisi ${divName}? Karyawan akan dialihkan ke Pusat.`)){
      setDivisions(divisions.filter(d => d !== divName));
      setUsers(users.map(u => u.division === divName ? { ...u, division: 'Pusat' } : u));
    }
  };
  const handleCreateUser = (e) => {
    e.preventDefault();
    if(users.some(u => u.nik === newUser.nik)) return alert('NIK sudah terdaftar!');
    const nameParts = (newUser.name || 'User').trim().split(/\s+/);
    const initials = nameParts.map(n => n[0]).join('').substring(0, 2).toUpperCase();
    setUsers([...users, { id: Date.now(), ...newUser, avatar: initials }]);
    setIsUserModalOpen(false);
    setNewUser({ nik: '', password: '', name: '', role: 'staff', division: '', position: '' });
  };
  const handleUpdateUser = (e) => {
    e.preventDefault();
    if(users.some(u => u.nik === editingUser.nik && u.id !== editingUser.id)) return alert('NIK dipakai oleh pengguna lain!');
  ...
  const handleDeleteUser = (userId) => {
    if(window.confirm('Hapus user ini?')) setUsers(users.filter(u => u.id !== userId));
  };
  ...
  const myTasks = safeTasks.filter(t => {
    ...
    // Ambil data siapa yang menugaskan dan siapa yang ditugaskan
    ...
    // 1. ROLE: ADMIN (Akses Penuh tanpa batas)
    if (currentUser.role === 'admin') return true;

    // 2. ROLE: DIREKSI (Lihat semua, kecuali tugas rahasia milik Admin)
    if (currentUser.role === 'direksi') {
      const isTaskAdmin = creator.role === 'admin' || assigneesData.some(u => u.role === 'admin');
      return !isTaskAdmin; 
    }

    // 3. ROLE: MANAGER (Divisi Sendiri + Lintas Divisi)
    if (currentUser.role === 'manager') {
      ...
      // Cek izin akses khusus (Lintas Divisi) dari Super Admin
      if (currentUser.crossDivision) {
        // Hanya bisa melihat tugas milik STAFF di divisi lain (tidak bisa lihat tugas Manager/Direksi lain)
        const isStaffTask = assigneesData.some(u => u.role === 'staff');
        return isMyOwnTask || isMyDivision || isStaffTask;
      }
      
      // Default Manager: Hanya divisi sendiri
      return isMyOwnTask || isMyDivision;
    }

    // 4. ROLE: STAFF (Hanya milik sendiri)
  ...
  const activeTasks = myTasks; // Menyamakan variabel untuk perhitungan Dashboard
  ...
  return (
    <div className="h-screen overflow-hidden bg-slate-50 font-sans text-slate-800 flex flex-col md:flex-row print:bg-white print:block">
      ...
      {/* MOBILE HEADER */}
      <div className="md:hidden flex items-center justify-between bg-white/90 backdrop-blur-md border-b border-slate-200 p-4 sticky top-0 z-30 print:hidden shadow-sm">
        <div className="flex items-center gap-2">
          <div className="bg-gradient-to-br from-indigo-500 to-indigo-800 p-1.5 rounded-lg shadow-sm"><LayoutDashboard className="w-5 h-5 text-white" /></div>
          <span className="font-black text-base tracking-tight text-slate-800 uppercase line-clamp-1">{sysConfig.brandName}</span>
        </div>
        <div className="flex gap-2 items-center">
            <button type="button" onClick={() => setIsNotifOpen(!isNotifOpen)} className="p-2 text-slate-600 relative">
                <Bell className="w-6 h-6" />
                {unreadNotifsCount > 0 && <span className="absolute top-1 right-1 bg-red-500 text-white text-[10px] font-bold w-4 h-4 flex items-center justify-center rounded-full border border-white">{unreadNotifsCount}</span>}
            </button>
            <button type="button" onClick={() => setMobileMenuOpen(true)} className="p-2 bg-slate-100 rounded-xl text-slate-600"><Menu className="w-6 h-6"/></button>
        </div>
      </div>
      ...
      {/* SIDEBAR NAVIGASI */}
      <aside className={`fixed md:relative top-0 bottom-0 left-0 w-72 md:w-72 bg-white/95 md:bg-white backdrop-blur-xl border-r border-slate-200/60 flex flex-col z-50 transform transition-transform duration-300 ease-in-out print:hidden ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <nav className="flex-1 p-4 md:p-5 space-y-1.5 overflow-y-auto custom-scrollbar">
            ...
            {/* Laporan Untuk Admin / Direksi disetel ke Mode Global = 'ALL' */}
        ...
        <div className="p-3 pb-32 md:p-6 md:pb-6 w-full max-w-[1600px] mx-auto h-full flex flex-col">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6 md:mb-8 print:hidden">
            ...
            <div className="flex flex-row items-center gap-2 md:gap-4 w-full md:w-auto overflow-visible">
               ...
               {activeTab === 'admin_users' && (
                  <button type="button" onClick={() => setIsUserModalOpen(true)} className="bg-slate-800 hover:bg-slate-900 text-white px-4 py-2.5 md:px-6 md:py-3 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg transition-all flex-1 md:flex-none text-xs md:text-sm">
                    <UserPlus className="w-4 h-4 md:w-5 h-5" /> Tambah Pengguna
                  </button>
               )}
            </div>
          </div>

          {/* TAB: INBOX PESAN (PUSAT DISKUSI) */}
          {activeTab === 'chat' && (
            <div className="bg-white md:rounded-2xl shadow-sm md:border border-slate-200 overflow-hidden flex flex-col md:flex-row h-[calc(100vh-100px)] md:h-[calc(100vh-140px)] animate-in fade-in duration-300 print:hidden -mx-4 md:mx-0 mt-[-16px] md:mt-0 border-t">
              ...
              <div className={`w-full md:w-1/3 border-r border-slate-200 flex flex-col bg-slate-50 ${selectedTask ? 'hidden md:flex' : 'flex'}`}>
                <div className="p-3 md:p-5 border-b border-slate-200 bg-white shrink-0">
                  <h3 className="font-black text-base md:text-lg text-slate-800 flex items-center gap-2"><MessageSquare className="w-4 h-4 md:w-5 md:h-5 text-indigo-500"/> Pesan Aktif</h3>
                </div>
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                  {myTasks
                    .filter(t => Array.isArray(t.comments) && t.comments.length > 0)
                    .sort((a, b) => b.comments[b.comments.length - 1].id - a.comments[a.comments.length - 1].id) // Urutkan dari chat terbaru
                    .map(task => {
                      const latestChat = task.comments[task.comments.length - 1];
                      const isMe = latestChat.userId === currentUser.id;
                      const isActive = selectedTask?.id === task.id; // Cek apakah chat ini sedang dibuka
                      
                      return (
                        <div 
                          key={task.id} 
                          // FITUR BARU: Klik untuk buka, klik lagi untuk tutup
                  ...
                  {myTasks.filter(t => Array.isArray(t.comments) && t.comments.length > 0).length === 0 && (
                    <div className="p-8 text-center text-slate-400 font-bold text-[10px] md:text-xs uppercase tracking-widest mt-10">Belum ada diskusi</div>
                  )}
                </div>
              </div>
              ...
              <div className={`w-full md:w-2/3 flex flex-col bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-fixed opacity-95 h-full ${!selectedTask ? 'hidden md:flex' : 'flex'}`}>
                {selectedTask ? (
                  <>
                    {/* Header Chat: Bisa diklik di HP untuk langsung menutup chat */}
                    <div 
                      onClick={() => window.innerWidth < 768 && setSelectedTask(null)}
                      className="px-3 py-2.5 md:px-6 md:py-4 border-b border-slate-200 bg-white shadow-sm flex items-center gap-2 md:gap-3 shrink-0 md:cursor-default cursor-pointer active:bg-slate-50"
                    >
                  ...
                  <Card className="p-5 md:p-6 border-0 shadow-sm bg-white">
                     ...
                     <div className="space-y-4 md:space-y-6">
                        {['high', 'medium', 'low'].map(prio => {
                          const count = activeTasks.filter(t => t.priority === prio && t.status !== 'done').length;
                          const total = activeTasks.filter(t => t.status !== 'done').length;
                          ...
                          return (
                            <div key={prio}>
                              <div className="flex justify-between text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 md:mb-2"><span>{labels[prio]}</span><span className="text-slate-800">{count} Tugas</span></div>
                              <div className={`w-full ${bgColors[prio]} rounded-full h-2 md:h-2.5 overflow-hidden shadow-inner`}><div className={`${colors[prio]} h-full rounded-full transition-all duration-500`} style={{width: `${total ? (count/total)*100 : 0}%`}}></div></div>
                            </div>
                          )
                        })}
                     </div>
          ...
          {activeTab === 'tasks' && (
            <div className="space-y-4 print:hidden animate-in fade-in duration-300">
               ...
               {/* TAMBAHAN KODE: UI Filter untuk Manajemen Pekerjaan */}
               <Card className="p-4 bg-white border-slate-200 shadow-sm mb-4">
                 <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                   <div className="flex items-center gap-2">
                     <Filter className="w-5 h-5 text-indigo-500" />
                     <span className="font-bold text-slate-700 text-sm">Filter Tanggal & Bulan Tugas:</span>
                   </div>
                   <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                     <div className="flex items-center gap-2">
                       <label className="text-xs font-bold text-slate-500">Bulan:</label>
                       <input type="month" value={taskFilterMonth} onChange={(e) => {setTaskFilterMonth(e.target.value); setTaskFilterDate('');}} className="px-3 py-2 border border-slate-200 rounded-lg text-sm font-bold focus:border-indigo-500 outline-none" />
                     </div>
                     <div className="flex items-center gap-2">
                       <label className="text-xs font-bold text-slate-500">Tanggal:</label>
                       <input type="date" value={taskFilterDate} onChange={(e) => {setTaskFilterDate(e.target.value); setTaskFilterMonth('');}} className="px-3 py-2 border border-slate-200 rounded-lg text-sm font-bold focus:border-indigo-500 outline-none" />
                     </div>
                     {(taskFilterMonth || taskFilterDate) && (
                       <button type="button" onClick={() => {setTaskFilterMonth(''); setTaskFilterDate('');}} className="text-xs font-bold text-red-500 hover:text-red-700 px-2 py-1 bg-red-50 rounded-lg">Reset</button>
                     )}
                   </div>
                 </div>
               </Card>
               ...
               <div className="grid grid-cols-1 gap-3 md:gap-3 max-h-[calc(100vh-240px)] overflow-y-auto custom-scrollbar pr-1 md:pr-2 pb-24 md:pb-10">
                  {myTasks.filter(t => {
                   // TAMBAHAN KODE: Logika filter array tasks
                   if (taskFilterMonth) return t.dueDate.startsWith(taskFilterMonth);
                   if (taskFilterDate) return t.dueDate === taskFilterDate;
                   return true;
                }).map(task => {
          ...
          {activeTab === 'laporan' && (
            <div className="space-y-4 md:space-y-6 animate-in fade-in duration-300 print:space-y-0">
              ...
              {/* TAMBAHAN KODE: Card Filter Gabungan (Periode Bulan & Pilihan Karyawan) */}
              <Card className="p-3 md:p-4 mb-3 md:mb-4 bg-white border-indigo-200 border-2 shadow-sm print:hidden">
                <div className="flex flex-col md:flex-row gap-4 items-start md:items-end justify-between">
                  ...
                  {/* Dropdown Filter Laporan (Admin / Manager / Direksi) */}
                  {(currentUser.role !== 'staff') && (
                    <div className="w-full md:w-1/2">
              ...
              {(() => {
                ...
                return (
                  // BUNGKUSAN BARU AGAR BISA DI-SCROLL DI LAYAR, TAPI FULL-SIZE SAAT DI PRINT
                  <div className="max-h-[calc(100vh-260px)] overflow-y-auto custom-scrollbar print:max-h-none print:overflow-visible pb-10 print:pb-0">
                    <Card className="p-0 border-0 shadow-sm bg-white print:shadow-none print:border-none print:w-full overflow-hidden print-page">
                      ...
                      {/* Header Kop Surat Laporan (Padding Web Dikecilkan, Padding Print Tetap) */}
                      ...
                      <div className="px-3 md:px-5 print:px-0 pb-4 md:pb-6 bg-white">
                        ...
                        {/* Signature Area (Hanya Muncul Saat di Print) */}
          ...
          {activeTab === 'division' && (currentUser.role !== 'staff') && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 print:hidden animate-in fade-in duration-300">
            ...
            <div className="space-y-4 md:space-y-6 animate-in fade-in duration-300">
              <Card className="border-0 shadow-sm overflow-hidden bg-white">
                 <div className="p-4 md:p-6 border-b border-slate-100 flex justify-between items-center">
                   <h3 className="font-black text-base md:text-xl text-slate-800 flex items-center gap-2"><Users className="w-4 h-4 md:w-5 md:h-5 text-indigo-500"/> Kelola Daftar Pengguna</h3>
                 </div>
                         ...
                         <tr key={u.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                            ...
                            <td className="p-3 md:p-5 text-right">
                              <div className="flex justify-end gap-1.5 md:gap-2">
                                  ...
                                  {u.id !== currentUser.id && <button type="button" onClick={() => handleDeleteUser(u.id)} className="p-1.5 md:p-2 text-red-600 bg-red-50 hover:bg-red-100 rounded-lg shadow-sm"><Trash2 className="w-3.5 h-3.5 md:w-4 md:h-4"/></button>}
                              </div>
                            </td>
                         </tr>
                       ))}
          ...
          {activeTab === 'admin_settings' && currentUser.role === 'admin' && (
            <div className="max-w-2xl space-y-4 md:space-y-6 animate-in fade-in duration-300">
              <Card className="p-5 md:p-8 border-0 shadow-sm bg-white">
                ...
                <div className="space-y-5 md:space-y-6">
                  ...
                  <div className="border-t border-slate-100 pt-5 md:pt-6 mt-5 md:mt-6">
                    <h4 className="font-bold text-slate-800 mb-3 md:mb-4 flex items-center gap-2 text-sm md:text-base"><Briefcase className="w-4 h-4 md:w-5 md:h-5 text-indigo-500"/> Manajemen Divisi</h4>
                    <div className="flex gap-2 md:gap-3 mb-4 md:mb-6">
                       <input type="text" value={newDivName} onChange={e => setNewDivName(e.target.value)} placeholder="Nama divisi baru..." className="flex-1 px-3 py-2 md:px-4 md:py-3 border-2 border-slate-200 rounded-xl focus:border-indigo-500 outline-none font-bold text-sm text-slate-800 bg-slate-50 focus:bg-white" />
                       <button type="button" onClick={handleAddDivision} className="px-4 py-2 md:px-6 md:py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs md:text-sm rounded-xl flex items-center justify-center gap-1.5 transition-all"><Plus className="w-3.5 h-3.5 md:w-4 md:h-4"/> Tambah</button>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3">
                      {divisions.map(d => (
                         <div key={d} className="flex items-center justify-between p-2.5 md:p-3 bg-white border border-slate-200 rounded-xl shadow-sm hover:border-indigo-300">
                           {editingDiv === d ? (
                             <div className="flex flex-1 gap-1.5 items-center">
                               <input autoFocus type="text" value={editDivInput} onChange={(e) => setEditDivInput(e.target.value)} className="w-full min-w-0 flex-1 px-2 py-1 text-xs md:text-sm font-bold border-2 border-indigo-200 rounded-lg outline-none focus:border-indigo-500" />
                               <button type="button" onClick={() => handleUpdateDivision(d)} className="p-1.5 bg-emerald-100 text-emerald-700 hover:bg-emerald-200 rounded-lg"><Check className="w-3.5 h-3.5"/></button>
                               <button type="button" onClick={() => setEditingDiv(null)} className="p-1.5 bg-slate-100 text-slate-600 hover:bg-slate-200 rounded-lg"><X className="w-3.5 h-3.5"/></button>
                             </div>
                           ) : (
                             <>
                               <span className="font-bold text-slate-700 truncate text-xs md:text-sm mr-2">{d}</span>
                               <div className="flex gap-1 md:gap-1.5 shrink-0">
                                 <button type="button" onClick={() => { setEditingDiv(d); setEditDivInput(d); }} className="p-1.5 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg"><Edit className="w-3.5 h-3.5"/></button>
                                 <button type="button" onClick={() => handleDeleteDivision(d)} className="p-1.5 text-red-600 bg-red-50 hover:bg-red-100 rounded-lg"><Trash2 className="w-3.5 h-3.5"/></button>
                               </div>
                             </>
                           )}
                         </div>
          ...
          {/* === MODAL 1: DETAIL TUGAS & LAMPIRAN === */}
          {selectedTask && activeTab !== 'chat' && (
            <div className="fixed inset-0 bg-slate-900/70 z-[70] flex justify-center md:items-center md:p-8 print:hidden">
              <div className="w-full h-full md:max-w-6xl md:h-[90vh] bg-white md:rounded-3xl shadow-2xl flex flex-col md:flex-row overflow-hidden animate-in slide-in-from-bottom-10 md:zoom-in duration-300">
                ...
                <div className="w-full md:w-1/2 flex flex-col bg-white border-b md:border-b-0 md:border-r border-slate-200 h-[50%] md:h-full">
                  ...
                  <div className="p-5 md:p-8 overflow-y-auto flex-1 space-y-6 md:space-y-8 custom-scrollbar bg-slate-50/30">
                    ...
                    {/* FITUR LAMPIRAN BUKTI PENYELESAIAN */}
                    <div className="bg-white p-4 md:p-6 rounded-2xl border border-slate-200 shadow-sm">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          <div className="relative">
                            {/* Attribut capture="environment" akan otomatis membuka Kamera Belakang di HP */}
                      ...
                      <div className="space-y-2 md:space-y-3 mt-4">
                        {(Array.isArray(selectedTask?.attachments) ? selectedTask.attachments : []).map(file => {
                          ...
                          // Syarat boleh hapus: Hanya jika dia sendiri yang mengupload, ATAU jika dia adalah Admin.
                          ...
                          return (
                            <div key={file?.id || Math.random()} className="flex items-center justify-between p-3 md:p-4 border border-slate-200 rounded-xl bg-slate-50 hover:bg-slate-100 transition-colors">
                              ...
                              <div className="flex gap-1.5 md:gap-2 shrink-0 ml-2">
                                ...
                                {/* Tombol Hapus (Hanya muncul untuk pemilik atau Admin) */}
                                {isMine && (
              ...
              <Card className="w-full h-full md:h-auto md:max-w-xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 md:zoom-in duration-300 border-0 shadow-2xl md:rounded-2xl">
                ...
                <form onSubmit={handleUpdateUser} className="flex flex-col flex-1 min-h-0">
                  <div className="overflow-y-auto custom-scrollbar flex-1 bg-white p-5 md:p-8 space-y-4 md:space-y-6">
                    ...
                    {/* FITUR KHUSUS ADMIN: IZIN LINTAS DIVISI UNTUK MANAGER */}
          ...
          <div className="md:hidden fixed bottom-4 left-4 right-4 bg-white/85 backdrop-blur-xl border border-white/60 shadow-[0_10px_40px_rgba(0,0,0,0.1)] rounded-[2rem] z-40 flex justify-around items-center px-4 py-2 print:hidden">
            <button type="button" onClick={() => navigateTo('dashboard')} className={`flex flex-col items-center justify-center w-full py-2 gap-1 rounded-xl transition-colors ${activeTab === 'dashboard' ? 'text-indigo-600' : 'text-slate-400'}`}>
              <LayoutDashboard className={`w-6 h-6 ${activeTab === 'dashboard' ? 'fill-indigo-100' : ''}`} />
              <span className="text-[9px] font-black tracking-wide">Beranda</span>
            </button>
            <button type="button" onClick={() => navigateTo('tasks')} className={`flex flex-col items-center justify-center w-full py-2 gap-1 rounded-xl transition-colors ${activeTab === 'tasks' ? 'text-indigo-600' : 'text-slate-400'}`}>
              <CheckSquare className={`w-6 h-6 ${activeTab === 'tasks' ? 'fill-indigo-100' : ''}`} />
              <span className="text-[9px] font-black tracking-wide">Tugas</span>
            </button>
            
            {/* Tombol Tengah (FAB) Melayang */}
            <div className="relative -top-7 flex-shrink-0 px-2">
               <button type="button" 
                  onClick={() => { if(activeTab === 'admin_users') setIsUserModalOpen(true); else setIsModalOpen(true); }} 
                  className="bg-gradient-to-br from-indigo-500 to-indigo-700 text-white p-4 rounded-full shadow-[0_10px_25px_rgba(79,70,229,0.4)] transform transition-transform hover:scale-105 active:scale-95 border-[5px] border-slate-50">
                 {activeTab === 'admin_users' ? <UserPlus className="w-6 h-6" /> : <Plus className="w-6 h-6" strokeWidth={3} />}
               </button>
            </div>

            {(currentUser.role === 'staff') ? (
              <button type="button" onClick={() => navigateTo('laporan')} className={`flex flex-col items-center justify-center w-full py-2 gap-1 rounded-xl transition-colors ${activeTab === 'laporan' ? 'text-indigo-600' : 'text-slate-400'}`}>
                <FileText className={`w-6 h-6 ${activeTab === 'laporan' ? 'fill-indigo-100' : ''}`} />
                <span className="text-[9px] font-black tracking-wide">Laporan</span>
              </button>
            ) : (
              <button type="button" onClick={() => navigateTo('division')} className={`flex flex-col items-center justify-center w-full py-2 gap-1 rounded-xl transition-colors ${activeTab === 'division' ? 'text-indigo-600' : 'text-slate-400'}`}>
                <Users className={`w-6 h-6 ${activeTab === 'division' ? 'fill-indigo-100' : ''}`} />
                <span className="text-[9px] font-black tracking-wide">Tim</span>
              </button>
            )}
            <button type="button" onClick={() => setMobileMenuOpen(true)} className="flex flex-col items-center justify-center w-full py-2 gap-1 rounded-xl text-slate-400">
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        @media print {
          ...
          /* KUNCI PERBAIKAN: overflow visible agar tidak terpotong ke halaman 2 dst */
